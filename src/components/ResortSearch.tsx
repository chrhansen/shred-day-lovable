...
import { AddResortSheet } from "@/components/AddResortSheet";
...
export function ResortSearch({ onSelect }: ResortSearchProps) {
  ...
  const handleStartAddResort = () => {
    setNewResortName(query);
    setOpen(false);
    setShowAddSheet(true);
  };

  const handleAddNewResort = (resort: { name: string; latitude?: number; longitude?: number }) => {
    // In production, this would call your backend API to create the resort with optional coordinates
    onSelect(resort.name);
